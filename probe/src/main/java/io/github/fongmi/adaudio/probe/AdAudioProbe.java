...
import io.github.fongmi.adaudio.probe.internal.media3.Media3ProbeEngine;
...
public final class AdAudioProbe implements Closeable {
    ...
    private final Media3ProbeEngine engine;
    ...
    private AdAudioProbe(Builder builder) {
        ...
        Handler handler;
        Media3ProbeEngine mediaEngine;
        try {
            thread.start();
            handler = new Handler(thread.getLooper());
            mediaEngine = new Media3ProbeEngine(builder.context.getApplicationContext(),
                    thread.getLooper(), hostPositionMs, builder.maxLookaheadMs,
                    new EngineListener());
    ...
    public long open(ProbeMedia media) {
        ...
        SessionStart start = callbackGate.update(() -> {
            synchronized (stateLock) {
                ...
                return new SessionStart(sessionId, next);
            }
        });
     ...
     * 内部 Media3 资源随后在专用 Looper 释放；重复调用无副作用。
     */
    @Override
    public void close() {
    ...
    private void requestHostPosition(long sessionId) {
        ...
        hostExecutor.tryExecute(() -> {
            ...
            try {
                long position = playbackClock.getCurrentPositionMs();
                if (position < 0L) throw new IllegalStateException("宿主返回了负时间轴");
        ...
        public void onRules(AdRuleSet loaded, boolean fromCache) {
            RuleTransition transition = callbackGate.update(() -> {
                synchronized (stateLock) {
                    ...
                    return new RuleTransition(sessionId, needsClock, next);
                }
            });
            if (transition == null) return;
            updateStatus(transition.status, true);
    ...
    private final class EngineListener implements Media3ProbeEngine.Listener {
        @Override
    ...
    private static final class SessionStart {
        ...
        SessionStart(long sessionId, ProbeStatus status) {
    ...
    private static final class RuleTransition {
        ...
        RuleTransition(long sessionId, boolean needsClock, ProbeStatus status) {
            this.sessionId = sessionId;
